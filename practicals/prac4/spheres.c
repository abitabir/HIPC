...
double random_number() {
	...
	return (MAX_RAND * (double) rand() / RAND_MAX);
}
...
int main(int argc, char *argv[]) {
	...
	for (int i = 0; i < N; i++) {
		...
		// for all of the j iterations as part of this i iteration
		...
		for (j = 0; j+4-1 < i; j += 4) {
			...
			/* The next two lines are only used for the faster branching version that was commented out */
			double d[4];
			...
			*/ 
			// NOTE: The code below is actually slightly slower than the non-branching AVX2 version of above..!
	...
	for (int i = 0; i < N; i++) {
		printf("%lf, %lf, %lf, %lf, %lf, %lf, %d\n", sphere.x[i], sphere.y[i], sphere.z[i], sphere.r[i], area[i], volume[i], (int) intersects[i]);
	}
